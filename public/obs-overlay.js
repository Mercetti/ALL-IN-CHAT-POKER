/**
 * OBS table overlay that mirrors players around the table with avatars and names.
 */
...
const socket = io(SOCKET_URL || undefined, {
  ...
  auth: {
    token: (typeof window !== 'undefined' && window.__USER_TOKEN__) || null,
  },
});
...
// Keep seat positions sticky across updates so avatars do not hop around
...
function renderSeats() {
  ...
  if (seatedPill) {
    seatedPill.textContent = `${overlayPlayers.length} players · ${seatNodes.length} seats`;
  }
}
...
socket.on('error', (err) => {
  ...
  if (window.Toast && err) {
    Toast.error(typeof err === 'string' ? err : 'Overlay error');
  }
});
