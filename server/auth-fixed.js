 ...
 * Added requireOwner middleware and improved role checking
 */
...
/**
 * Check if request is authorized as owner (NEW FUNCTION)
 * @param {Object} req - Express request or Socket.IO handshake
 * @returns {boolean}
 */
function isOwnerRequest(req) {
  try {
    // Check JWT cookie first (most secure)
    const jwtPayload = extractJWT(req);
    if (jwtPayload && jwtPayload.owner) return true;

    // Check permanent token
    const token = extractAdminToken(req);
    if (config.OWNER_TOKEN && token === config.OWNER_TOKEN) return true;

    // Check user profile for owner role
    const userToken =
      extractAdminToken(req) ||
      (req && req.handshake && req.handshake.auth && req.handshake.auth.token) ||
      (req && req.auth && req.auth.token) ||
    ...
    logger.debug('Owner check failed', { error: err.message });
  }
...
/**
 * Middleware: Check owner authorization (NEW FUNCTION)
 * @param {Object} req - Express request
 * @param {Object} res - Express response
 * @param {Function} next - Express next
 */
function requireOwner(req, res, next) {
  // Debug: Log all headers
  logger.debug('owner request headers', { 
    authorization: getHeader(req, 'authorization'),
    cookie: getHeader(req, 'cookie'),
    ip: req.ip 
  });
  ...
  if (!isOwnerRequest(req)) {
    logger.warn('Unauthorized owner access attempt', { ip: req.ip });
    return res.status(403).json({ error: 'not authorized' });
  }
  next();
}
...
module.exports = {
  ...
  extractJWT,
  isAdminRequest,
  isOwnerRequest, // NEW: Owner-specific check
  requireOwner, // NEW: Owner-specific middleware
  signJWT,
  signUserJWT,
  createAdminJWT,
