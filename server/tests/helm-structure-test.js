...
// Test 6: Check for TypeScript syntax errors
console.log('\n✅ Testing TypeScript compilation...');
try {
  const { execSync } = require('child_process');
  const result = execSync('npx tsc --noEmit --skipLibCheck server/helm/index.ts', { 
    cwd: projectRoot,
    stdio: 'pipe'
  });
  console.log('✅ TypeScript compilation successful');
} catch (error) {
  console.log('⚠️ TypeScript compilation issues (expected during development)');
}
