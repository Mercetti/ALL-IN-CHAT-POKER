...
function createIncidentRouter({ db, logger }) {
...
async function triggerImmediateResponse(incident) {
  try {
...
async function executeIncidentAction(action, incident) {
  const actionMap = {
    'freeze': async () => {
      db.prepare('INSERT OR REPLACE INTO system_state (key, value) VALUES (?, ?)').run('mode', 'FROZEN');
...
async function enableIncidentLearning(incidentId, resolution) {
  try {
...
async function updateEvaluationModels(incidentId, learningData) {
