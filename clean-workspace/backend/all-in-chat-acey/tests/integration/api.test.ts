...
describe('API Integration Tests', () => {
  ...
  describe('Health Check API', () => {
    it('should return 200 for health check', async () => {
      ...
      mockApp.get.mockImplementation((path: string, handler: Function) => {
        if (path === '/health') {
          handler(testUtils.createMockRequest(), testUtils.createMockResponse());
        }
      });

      const response = {
        status: 200,
        body: testUtils.createMockUser(),
      };
  ...
  describe('Notification API', () => {
    it('should send a notification', async () => {
      const notificationData = testUtils.createMockNotification({
  ...
  describe('WebSocket API', () => {
    ...
    it('should validate WebSocket messages', async () => {
      const validMessage = testUtils.createMockMeshMessage({
