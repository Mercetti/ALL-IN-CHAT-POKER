...
export const SkillLibraryScreen: React.FC<Props> = ({ userRole, userTier, onSkillExecute }) => {
  ...
  const loadSkills = async () => {
    setLoading(true);
    try {
      const fetchedSkills = await ApiService.getSkills(userRole);
  ...
  const handleSkillExecute = async (skillName: string) => {
    try {
      const result = await ApiService.executeSkill({
        skillName,
        input: { userRole, timestamp: new Date().toISOString() },
        dryRun: false
      }, userRole);

      if (result.success) {
        Alert.alert('Success', `Skill "${skillName}" executed successfully`);
        
        // Send notification for skill execution
        await NotificationService.sendNotification({
          title: 'Skill Executed',
          body: `${skillName} completed successfully`,
          type: 'info',
          priority: 'medium',
          data: { skillName, result }
        });
      } else {
        Alert.alert('Error', `Skill execution failed: ${result.result}`);
      }
    } catch (error) {
      console.error('Skill execution failed:', error);
      Alert.alert('Error', 'Failed to execute skill');
    }
  };
  ...
  const renderSkillItem = ({ item }: { item: Skill }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => canAccess && handleSkillExecute(item.name)}
        disabled={!canAccess}
      >
